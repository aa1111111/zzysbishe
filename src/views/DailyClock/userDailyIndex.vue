    ...
    <div class="jz">
      ...
      >
        <el-form-item label="1.目前所在地区" >
          <v-distpicker :disabled=disabled
        ...
        >
          <el-radio-group v-model="form.hasTravelMediumHighRiskAreas" :disabled=disabled>
        ...
        <el-form-item class="btn">
          <el-button v-show="this.gai == null && this.disabled != disabled" type="primary" @click="save"
            >立即创建</el-button
          >
...
export default {
  ...
  mounted() {
    ...
    if(this.$route.query.record != null){
        ...
        this.addressprovince=this.$route.query.record.currentLocation.split("-")[0]
    ...
    getHealthyRecordInfo() {
      userDailyApi.getHealthyRecordInfo(this.recordId).then((response) => {
        if (response.code == 20000) {
          ...
          this.addressprovince=response.data.healthyRecord.currentLocation.split("-")[0]
          this.addresscity=response.data.healthyRecord.currentLocation.split("-")[1]
          this.addressdist=response.data.healthyRecord.currentLocation.split("-")[2]
        }
      });
    },
    updateHealthyRecord() {
